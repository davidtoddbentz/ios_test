    ...
    class AwsDeviceFarmAction < Action
      def self.run(params)
        ...
        if params[:test_binary_path]
          ...
          test_upload = create_project_upload project, test_path, 'APPIUM_JAVA_TESTNG_TEST_PACKAGE'

          # Upload the test binary.
          Helper.log.info 'Uploading the Appium Test Package. ☕️'
          upload test_upload, test_path

          # Wait for test upload to finish.
          Helper.log.info 'Waiting for the TestPackage  upload to succeed. ☕️'
      ...
      def self.schedule_run(project, device_pool, upload, test_upload)
        ...
        if test_upload
          test_hash[:type]             = 'APPIUM_JAVA_TESTNG'
