...
if [ "$oclint" = "on" ]; then
    ...
    nestedBlockDepthThreshold=7
	runCommand no oclint-json-compilation-database $includedCommandLineFlags -- -rc LONG_LINE=$longLineThreshold -rc NESTED_BLOCK_DEPTH=$nestedBlockDepthThreshold -max-priority-1 $maxPriority -max-priority-2 $maxPriority -max-priority-3 $maxPriority -report-type pmd -o sonar-reports/oclint.xml
else
