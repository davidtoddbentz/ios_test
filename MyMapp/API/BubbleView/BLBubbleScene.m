...
- (void)reload
{
    ...
    for (int i=0; i<numberOfBubbles; i++) {
        ...
        node.fillColor = [self.bubbleDataSource bubbleScene:weakSelf bubbleFillColorForIndex:i];
        node.label.fontColor = [SKColor whiteColor];
             ...
             toState:(BLBubbleNodeState)nextState
{
    ...
    _magneticField.strength = -300000;
    SKAction *action = [SKAction waitForDuration:0.5];
        [bubble runAction:action completion:^{
    }];
           ...
           withEvent:(UIEvent * __nullable)event
{
    ...
    //This is so taps, long presses and pans don't intSerfere with each other
           ...
           withEvent:(UIEvent * __nullable)event
{
    ...
    if (self.touchesMoved || self.touchTapped)
        return;
